...
def FHC_MC_approach(n,N,S_matrix,T,init_price,init_delta,init_spot,nbr_hedges,freq_obs,BP,AT,coupon,step_size=252):
    ...
    for hedge_date in hedge_dates[:-1]:
        ...
        S_matrix, S_matrix_up, S_matrix_down = [
            path_generator().generate_paths(n, paths, curve, dividends_matrix, N, (T - hedge_date / 252), init_spot,
                                            list_params_matrix, vol_type, 'Delta', shock)
            for shock in ['None','Up', 'Down']]
        ...
        if position != prev_pos :
            active, active_up, active_down = [np.min(mat[:, :], axis=1) for mat in [sm, sm_up, sm_down]]
            called_paths = np.where(pm_up[:, 0] != 0)[0]
            #pm, pm_up, pm_down = [np.delete(payoff, called_paths, axis=0) for payoff in [pm, pm_up, pm_down]]
            print(called_paths)
            print(called_paths.shape[0])

            k = k+1
        matrix, matrix_up, matrix_down = [S_matrix, S_matrix_up, S_matrix_down]
        hedge_spot = matrix[:,0]
        ...
        eval_mat, eval_mat_up, eval_mat_down = [mat[:,obs_dates_new]/matrix[:,0][:, np.newaxis] for mat in [matrix,matrix_up,matrix_down]]
        ...
        #pm, pm_up, pm_down = [np.delete(payoff, called_paths, axis=0) for payoff in [pm, pm_up, pm_down]]
        if position != prev_pos:
            pm_up = np.delete(pm_up,called_paths,axis = 0)
            prev_pos = position
        print(pm_up)
